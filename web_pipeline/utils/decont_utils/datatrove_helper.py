...
class NGramsDecontConfig:
    ...
    # for math we do not do number normalize
...
class NGramsDecontIndexer(PipelineStep):
    ...
    Ways to provide task data:
      ...
      #FIXME
      @fan, deprecated, we do not use lighteval in megamath
    ...
    def compute_hashes(self, label: str, query: str | None = None) -> list[int]:
        ...
        label_tokens = simplify_text(label, self.config.norm_config).lower().split()
        ...
        if query is not None:
            ...
            query_tokens = simplify_text(query, self.config.norm_config).lower().split()
    ...
    def run(self, data: DocumentsPipeline = None, rank: int = 0, world_size: int = 1):
        ...
        for task_name, task in self.task_dict.items():
            for eval_doc in task:
                try:
                    golds = eval_doc["label"] if isinstance(eval_doc["label"], list) else [eval_doc["label"]]
...
class NGramsDecontFilter(BaseFilter):
    ...
    def filter(self, doc: Document) -> bool | Tuple[bool, str]:
        ...
        text_tokens = simplify_text(doc.text, self.config.norm_config).lower().split()
        ...
        for n_gram in map(" ".join, ngrams_to_compute):
            ...
            #FIXME
            # @fan, insert punctuation check to avoid empty ngrams
            punc_check_config = self.config.norm_config
            punc_check_config.remove_punctuation = True
            punc_check_text = simplify_text(n_gram, punc_check_config)
            if punc_check_text == "":
                return True
